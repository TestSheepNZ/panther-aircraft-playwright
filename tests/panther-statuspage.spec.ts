...
test("After equipment load, aircraft altitude is set to 0", async ({
  page,
}) => {
  await equipmentPage.loadButton.click();
...
test("After equipment load, fuel used last leg is set to 0", async ({
  page,
}) => {
  await equipmentPage.loadButton.click();
...
test("After entering fuel of 2145kg on equipment page, status page says 2145kg fuel available", async ({
  page,
}) => {
  const fuelVal = "2145";
...
test("Aircraft has an all up weight of 17000 if loaded with 6000kg fuel, 8 missiles, 6 dumb bombs, ext fuel tank", async ({
  page,
}) => {
  const expectedAllUpWeight = "17000";
  await equipmentPage.setAircraftEquipment(
    "6000",
    "8",
    "6",
    false,
